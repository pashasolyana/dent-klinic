<template>
  <section class="wrap">
    <div class="card">
      ...
      <div class="pageGrid">
        ...
        <aside class="side">
          <div class="secHead">
            ...
            <div class="secActions">
              <button class="btn ghost" @click="loadCats" :disabled="busyCats">
                Обновить
              </button>
        ...
        <main class="main">
          <div class="secHead">
            ...
            <div class="secActions">
              <button
                class="btn ghost"
                @click="loadWorks"
                :disabled="busyWorks || !currentSlug"
              >
                Обновить
              </button>
              <button
          ...
          <div v-if="!currentSlug" class="panel muted">
            Выберите или создайте категорию слева.
          </div>

          <!-- CATEGORY EDIT -->
          <form
            v-if="currentCat"
            class="panel formGrid"
          ...
          >
            ...
            <label class="field spanAll">
              ...
              <div class="uploadRow uploadGroup">
                ...
                <input
                  ...
                  accept="image/*"
                  hidden
                  @change="onPickCatEdit"
                />
                <button
                  type="button"
                  class="btn groupBtn"
                  @click="fileCatEdit?.click()"
                >
                  Загрузить
                </button>
              </div>
            ...
            <div class="actions right spanAll">
              <button class="btn" :disabled="busyCats">
                Сохранить категорию
              </button>
            </div>
          </form>
          ...
          <div class="panel" v-else-if="works.length">
            <div class="gTable">
              ...
              <div class="gRow" v-for="(w, i) in works" :key="w.id">
                <div class="cell">
                  <input v-model="w.title" class="inp inp-sm" />
                </div>
                <div class="cell numCell">
                  <input v-model.number="w.price" class="inp inp-sm inp-num" />
                </div>
                <div class="cell">
                  <input v-model="w.note" class="inp inp-sm" />
                </div>
                <div class="cell actions">
                  <button
                    class="icon"
                    title="Вверх"
                    @click="reorderWorks(i, -1)"
                  >
                    ▲
                  </button>
                  <button
          ...
          <div class="panel muted" v-else-if="currentSlug">
            Работ пока нет — добавьте первую.
          </div>

          <div class="actions end">
            <span v-if="err" class="err">{{ err }}</span>
            <span v-else-if="updatedAt" class="hint"
              >Обновлено: {{ updatedAt }}</span
            >
          </div>
        </main>
      </div>
    ...
    <div
      v-if="showCreateCat"
      class="modal"
      @keydown.esc="showCreateCat = false"
    >
        ...
        <form class="modal__form" @submit.prevent="createCat">
          <div class="formGrid modalGrid">
            <label class="field">
              <span class="label">Название</span>
              <input
                v-model="newCat.title"
                class="inp"
                required
                maxlength="120"
              />
              <span class="slugHint"
                >slug: <code>{{ newCat.slug || '—' }}</code></span
              >
            </label>
...
const currentCat = computed<Cat | null>(
  () => categories.value.find((c) => c.slug === currentSlug.value) || null
);
...
const updatedAt = computed(() => {
  const last = works.value.reduce<string | null>(
    (acc, w) =>
      !acc || (w.updatedAt && w.updatedAt > acc) ? w.updatedAt || acc : acc,
    null
  );
...
function openAddWork() {
  if (currentSlug.value) showAddWork.value = true;
}
...
async function loadCats() {
  ...
  err.value = '';
  try {
    categories.value = await $fetch<Cat[]>('/api/admin/services', {
      credentials: 'include',
    });
    if (!currentSlug.value && categories.value.length)
      currentSlug.value = categories.value[0].slug;
  } catch (e: any) {
    err.value = e?.data?.message || 'Ошибка загрузки категорий';
  } finally {
...
async function loadWorks() {
  ...
  busyWorks.value = true;
  err.value = '';
  try {
    works.value = await $fetch<Work[]>(
      `/api/admin/services/${currentSlug.value}/works`,
      { credentials: 'include' }
    );
  } catch (e: any) {
    err.value = e?.data?.message || 'Ошибка загрузки работ';
  } finally {
    busyWorks.value = false;
  }
}

/* CREATE CAT (modal) */
const newCat = reactive<any>({
  title: '',
  slug: '',
  href: '',
  img: '',
...
async function createCat() {
  ...
  err.value = '';
  try {
    const payload: any = { ...newCat };
    if (!payload.slug) delete payload.slug;
    const created = await $fetch<Cat>('/api/admin/services', {
      method: 'POST',
      body: payload,
      credentials: 'include',
    });
...
watch(currentCat, (c) => {
  if (c) Object.assign(catForm, JSON.parse(JSON.stringify(c)));
});
...
async function saveCat() {
  ...
  try {
    ...
    const updated = await $fetch<Cat>(
      `/api/admin/services/${currentCat.value.slug}`,
      { method: 'PUT', body: rest, credentials: 'include' }
    );
    const i = categories.value.findIndex(
      (x) => x.slug === currentCat.value!.slug
    );
    if (i >= 0) categories.value[i] = updated;
  } catch (e: any) {
    err.value = e?.data?.message || 'Ошибка сохранения категории';
  } finally {
    busyCats.value = false;
  }
}
...
async function removeCat(slug: string) {
  ...
  try {
    ...
    if (currentSlug.value === slug)
      currentSlug.value = categories.value[0]?.slug || null;
  } catch (e: any) {
...
async function createWork() {
  ...
  try {
    ...
    const created = await $fetch<Work>(
      `/api/admin/services/${currentSlug.value}/works`,
      {
        method: 'POST',
        body: payload,
    ...
    err.value = e?.data?.message || 'Не удалось добавить работу';
  } finally {
    busyWorks.value = false;
  }
}
...
async function removeWork(id: string) {
  ...
  await $fetch(`/api/admin/services/${currentSlug.value}/works/${id}`, {
    method: 'DELETE',
    credentials: 'include',
  });
...
async function reorderWorks(i: number, dir: number) {
  const j = i + dir;
  if (j < 0 || j >= works.value.length) return;
  const arr = works.value;
  [arr[i], arr[j]] = [arr[j], arr[i]];
  await $fetch(`/api/admin/services/${currentSlug.value}/works/reorder`, {
    method: 'PUT',
    body: arr.map((x) => x.id),
    credentials: 'include',
  });
...
async function onPickCat(e: Event) {
  const f = (e.target as HTMLInputElement).files?.[0];
  if (!f) return;
  newCat.img = await uploadFile(f);
}
async function onPickCatEdit(e: Event) {
  const f = (e.target as HTMLInputElement).files?.[0];
  if (!f) return;
  catForm.img = await uploadFile(f);
}
...
@media (min-width: 1400px) {
  .gTable {
    --col-title: minmax(320px, 1fr);
    --col-price: 120px;
    --col-note: minmax(220px, 0.9fr);
    --col-actions: 220px;
  }
}
.gHead,
.gRow {
  display: grid;
  grid-template-columns: var(--col-title) var(--col-price) var(--col-note) var(
      --col-actions
    );
  gap: 10px;
}
